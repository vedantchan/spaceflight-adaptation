...
%
% $Date: 2017/11/02 20:39:43 $
% $Revision: 5.63 $
%
% $Log: crqa.m,v $
% Revision 5.63  2017/11/02 20:39:43  marwan
% fix bug when 1st column of input variable is time vector
%
% Revision 5.62  2017/09/18 20:12:06  marwan
% crucial fixes for bivariate analysis
%
% Revision 5.61  2017/07/06 16:29:29  marwan
...
%     w=Nx-(m-1)*t; wstep=1;; 
   ...
   if nonorm
       xTemp = x(:,time_scale_flag_x+1:end);
       yTemp = y(:,time_scale_flag_x+1:end);
       xMean = mean(xTemp);
       yMean = mean(yTemp);
       xStd = std(xTemp);
       yStd = std(yTemp);
       x(:,time_scale_flag_x+1:end) = (xTemp - repmat(xMean,length(x),1)) ./ repmat(xStd,length(x),1);
       y(:,time_scale_flag_x+1:end) = (yTemp - repmat(yMean,length(y),1)) ./ repmat(yStd,length(y),1);
   end
...
%%%%%%%%%%% plots
  ...
  if size(x_var,1) > 2, linkaxes(h,'x'), end
...
if ~nogui
    ...
    index=[1,2,3,5,6,7,11,10,18,19,20];
    ...
    linkaxes([h_axes.h h_crqa_axes_Data],'x')
    if size(Y,1)==1, linkaxes([h_axes.h h_crqa_axes_Data],'off'), end
...
%        end
   ...
   if length(Y(:,1)) > 1, 
       ...
       set(h_crqa_axes_Data,'xlim',[xscale(1) xscale(end)])
   end
