...
if nogui~=2, hw=waitbar(0,['0/',num2str(Nx-w)]);set(hw,'Name','Please Wait!'); drawnow; end
...
for i=1:wstep:Nx-w; 
     ...
     if plugin_exist & ( method_n < 4 ) & length(x) == length(y) 
         ...
         if nogui~=2, waitbar(i/(Nx-w),hw,[num2str(i),'/',num2str(Nx-w)]); drawnow, end
