...
function [RR,DET,ENTR,L] = Recu_RQA_mjs(RM,thr)
% Recurrence quantification analysis of recurrence plots
% RM:  the Recurrence Matrix (non-normalized distances)
% I:   the indication marks (I=0 RP is the symmetry matrix
%                            I=1 RP is the asymmetry matrix)  
% RR:   Recurrence rate RR, The proportion of recurrence points in an RP
%       Corresponds to the correlation sum;
% DET:  Determinism DET, The proportion of recurrence points which form
%       diagonal lines
...
% **** October 2018 MS
% My RM is a recurrence matrix of non-normalized distances.
% Normalize first, as in RexPlt
a=RM;
amin = min(min(a));
amax = max(max(a));
a = ((a-amin)/(amax-amin));
